...
app.post("/api/" + cst.API_VERSION + "/admin/campaign", function(req, res) {
  ...
  mysql.con.query("insert into campaign set ?", campaign, function(
    error,
    results,
    fields
  ) {
...
app.post("/api/" + cst.API_VERSION + "/admin/hot", function(req, res) {
  ...
  mysql.con.beginTransaction(function(error) {
    ...
    mysql.con.query("insert into hot set ?", hot, function(
      error,
      results,
      fields
    ) {
      ...
      for (let i = 0; i < productIds.length; i++) {
        hotProductMapping.push([hotId, parseInt(productIds[i])]);
      }
      mysql.con.query(
        "insert into hot_product(hot_id,product_id) values ?",
        [hotProductMapping],
...
app.get("/api/" + cst.API_VERSION + "/marketing/hots", function(req, res) {
  let query =
    "select hot.title as title,hot_product.product_id as product_id from hot,hot_product where hot.id=hot_product.hot_id order by hot.id";
  mysql.con.query(query, function(error, results, fields) {
    if (error) {
      res.send({ error: "Database Query Error" });
    } else {
      ...
      for (let i = 0; i < results.length; i++) {
        hot = data.find(hot => {
          return hot.title === results[i].title;
        });
        if (hot) {
      ...
      for (let i = 0; i < data.length; i++) {
        listProducts(
          { where: " where id in (" + data[i].products.join(",") + ")" },
...
app.post("/api/" + cst.API_VERSION + "/user/signin", function(req, res) {
  ...
  if (data.provider === "native") {
      ...
      mysql.con.query(
        ...
        function(error, results, fields) {
          ...
          let commitCallback = function(error) {
            ...
            if (user === null) {
              ...
              res.send({
                data: {
                  access_token: user.access_token,
                  access_expired: Math.floor(
                    (user.access_expired - now) / 1000
                  ),
                  user: {
                    id: user.id,
                    provider: user.provider,
          ...
          if (results.length === 0) {
            ...
            mysql.con.query(
              query,
              [user.access_token, user.access_expired, user.id],
              function(error, results, fields) {
                if (error) {
                  res.send({ error: "Database Query Error" });
                  return mysql.con.rollback(function() {
    ...
    getFacebookProfile(data.access_token)
      .then(function(profile) {
        if (!profile.id || !profile.name || !profile.email) {
          res.send({
            error: "Permissions Error: id, name, email are required."
          });
          return;
        }
        mysql.con.beginTransaction(function(error) {
          ...
          mysql.con.query(
            "select id from user where email = ? and provider = ?",
            [profile.email, data.provider],
            function(error, results, fields) {
              if (error) {
                res.send({ error: "Database Query Error" });
                return mysql.con.rollback(function() {
              ...
              if (results.length === 0) {
                ...
                query =
                  "update user set name = ?, access_token = ?, access_expired = ? where email = ?";
                query = mysql.core.format(query, [
                  user.name,
                  user.access_token,
                  user.access_expired,
                  user.email
...
let createPayment = function(payment, callback) {
  mysql.con.beginTransaction(function(error) {
    ...
    mysql.con.query("insert into payment set ?", payment, function(
      error,
      results,
      fields
    ) {
      ...
      mysql.con.query("update order_table set status = ?", [0], function(
        error,
        results,
        fields
      ) {
